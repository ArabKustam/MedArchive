// Страница «Клиники-партнёры» (FSD: pages/partners-list).
// Табличный список всех партнёров с фильтром по городу, поиском и пагинацией.

import { Link } from "@tanstack/react-router";
import { useMemo, useState } from "react";
import { AppLayout } from "@/widgets/app-layout";
import { Pager } from "@/shared/ui/Pager";
import { PAGE_SIZE_PARTNERS } from "@/shared/config/constants";
import { cities, partners } from "@/entities/partner";
...
export function PartnersListPage() {
  ...
  // Применяем фильтры (поиск по названию + город).
  ...
  const pageRows = filtered.slice((page - 1) * PAGE_SIZE_PARTNERS, page * PAGE_SIZE_PARTNERS);
        ...
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              ...
              <th className="px-4 py-3 text-right text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Услуг</th>
              <th className="px-4 py-3 text-right text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Документов</th>
            ...
            {pageRows.map((p) => (
              <tr key={p.id} className="transition-colors hover:bg-muted/30">
                <td className="px-4 py-3 text-sm font-medium">
                  <Link to="/partners/$id" params={{ id: p.id }} className="text-brand hover:underline">
                    {p.name}
                  </Link>

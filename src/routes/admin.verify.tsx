...
import { verificationQueue, type VerificationItem } from "../lib/mock-data";
...
function AdminVerify() {
  ...
  const [activeId, setActiveId] = useState<string>(verificationQueue[0]?.id ?? "");
  const [selectedCandidate, setSelectedCandidate] = useState<string>(
    verificationQueue[0]?.candidates[0]?.serviceId ?? "",
  );
